 ...
 */
function theme_medilearn_process_css($css, $theme) {

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_medilearn_set_logo($css, $logo);
    ...
    $css = theme_medilearn_set_customcss($css, $customcss);
 ...
 */
function theme_medilearn_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_medilearn_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $theme = theme_config::load('medilearn');
 ...
 */
function theme_medilearn_set_customcss($css, $customcss) {
 ...
 *
 * Do not add Medilearn specific logic in here, child themes should be able to
 ...
 */
function theme_medilearn_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
    ...
    if (!empty($page->theme->settings->logo)) {
        $return->heading = html_writer::tag('div', '', array('class' => 'logo suman'));
    } else {
...
function theme_medilearn_get_setting($setting, $format = false) {
    global $CFG;
...
/**
 * All theme functions should start with theme_medilearn_
 * @deprecated since 2.5.1
 */
function medilearn_process_css() {
 ...
 * @deprecated since 2.5.1
 */
function medilearn_set_customcss() {
    throw new coding_exception('Please call theme_'.__FUNCTION__.' instead of '.__FUNCTION__);
}
