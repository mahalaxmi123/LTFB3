...
/**
 * Medilearn core renderers.
 *
 * @package    theme_medilearn
 * @copyright  2015 Frédéric Massart - FMCorz.net
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_medilearn_core_renderer extends theme_bootstrapbase_core_renderer{
    ...
    public function context_header($headerinfo = null, $headinglevel = 1) {
        ...
        if ($this->should_render_logo($headinglevel)) {
            ...
            $logoheader = html_writer::start_tag('div', array('class' => 'header-in'));
    ...
    public function full_header() {
	 ...
	 public function render_social_network($socialnetwork) {
        if (theme_medilearn_get_setting($socialnetwork)) {
            $icon = $socialnetwork;
            $socialhtml = html_writer::start_tag('a', array(
                'class' => $socialnetwork,
                'onclick' => "window.open('" . theme_medilearn_get_setting($socialnetwork) . "')",
                'title' => get_string($socialnetwork, 'theme_medilearn'),
            ));
